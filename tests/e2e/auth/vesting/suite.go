...
func (s *E2ETestSuite) TestNewMsgCreateVestingAccountCmd() {
	...
	for name, tc := range testCases {
		...
		s.Run(name, func() {
			...
			bw, err := clitestutil.ExecTestCLICmd(clientCtx, cli.NewMsgCreateVestingAccountCmd(address.NewBech32Codec("cosmos")), tc.args)
			if tc.expectErr {
				s.Require().Error(err)
			} else {
				s.Require().NoError(err)
...
func (s *E2ETestSuite) TestNewMsgCreatePermanentLockedAccountCmd() {
	...
	testCases := map[string]struct {
		...
		"create a permanent locked account": {
			args: []string{
				sdk.AccAddress("addr4_______________").String(),
				sdk.NewCoins(sdk.NewCoin(s.cfg.BondDenom, math.NewInt(100))).String(),
				fmt.Sprintf("--%s=%s", flags.FlagFrom, val.Address),
				fmt.Sprintf("--%s=true", flags.FlagSkipConfirmation),
				fmt.Sprintf("--%s=%s", flags.FlagBroadcastMode, flags.BroadcastSync),
				fmt.Sprintf("--%s=%s", flags.FlagFees, sdk.NewCoins(sdk.NewCoin(s.cfg.BondDenom, math.NewInt(10))).String()),
	...
	for name, tc := range testCases {
		...
		s.Run(name, func() {
			...
			bw, err := clitestutil.ExecTestCLICmd(clientCtx, cli.NewMsgCreatePermanentLockedAccountCmd(address.NewBech32Codec("cosmos")), tc.args)
			if tc.expectErr {
				s.Require().Error(err)
			} else {
				s.Require().NoError(err)
