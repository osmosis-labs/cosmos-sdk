...
func TestStrategies(t *testing.T) {
	...
	for name, tc := range testcases {
		...
		t.Run(name, func(t *testing.T) {
			...
			snapshotHeight := int64(tc.snapshotInterval - 1)
			...
			for curHeight := int64(0); curHeight < 110000; curHeight++ {
				...
				default:
					if curHeight > int64(curKeepRecent) && curHeight%int64(curStrategy.Interval) == 0 {
						...
						if tc.snapshotInterval > 0 && snapshotHeight < pruningHeightExpected {
							pruningHeightExpected = snapshotHeight
						}
